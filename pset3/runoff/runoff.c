...
void tabulate(void)
{
    ...
    for (int j = 0; j < voter_count; j++) //make the second choice of the voters voting for the eliminated one move forward
    {
        ...
        
        {
            ...
            {
                for (int l = k + 1; l < voter_count; l++)
                {
                    if (!(preferences[j][l] == -1))
                    {
                        preferences[j][k] = preferences[j][l];
                        preferences[j][l] = -1;
                        break;
                    }
                }
...
bool print_winner(void)
{
    ...
    {
        if (max_vote == candidates[a].votes && max_vote > floor(0.5 * voter_count))
        {
            printf("%s\n", candidates[a].name);
            return true; 
        }
